...
export function onCSSFunction(node: FunctionNode, decl: Declaration, result: Result, preserve: boolean) {
...
type ColorMix = {
	...
	colorANode: Node,
	colorABlendNode: Node,
	colorBNode: Node,
	colorBBlendNode: Node,
}

function colorFunctionContents(nodes): ColorMix | null {
	if (!nodes[0] || nodes[0].type !== 'word' || nodes[0].value !== 'in') {
		return null;
	}
